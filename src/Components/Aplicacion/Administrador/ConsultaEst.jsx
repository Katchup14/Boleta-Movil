...
const ConsultaEst = () => {
  ...
  useEffect(() => {
    const obtenerEstudiantesYCursos = async () => {
      try {
        // Obtener estudiantes
        ...
        const cursosPromises = estudiantesData.map(async (estudiante) => {
          const cursosQuery = query(collection(db, 'curso_Inscrito'), where('id_Estudiante', '==', estudiante.id));
          const cursosSnapshot = await getDocs(cursosQuery);
          const cursosIds = cursosSnapshot.docs.map(doc => doc.data().id_curso);
          const cursosConNombre = await Promise.all(cursosIds.map(async (id_curso) => {
            const cursoDocRef = doc(db, 'cursos', id_curso);
            const cursoDoc = await getDoc(cursoDocRef);
            return cursoDoc.exists() ? cursoDoc.data().Nombre : null;
          }));
          return { id: estudiante.id, cursos: cursosConNombre.filter(nombre => nombre !== null) };
        });
    ...
    obtenerEstudiantesYCursos();
  }, []);

  const mostrarCursos = (id) => {
    const cursosDelEstudiante = cursos[id];

    if (cursosDelEstudiante && cursosDelEstudiante.length > 0) {
      setUniqueCurso(cursosDelEstudiante);
    } else {
      setUniqueCurso([]);
    }
    setModal(true);
        ...
        <ScrollView>
          <View style={styles.table}>
            ...
            {estudiantes.map((estudiante, index) => (
              <View key={estudiante.id} style={styles.row}>
                <Text style={[styles.cell, styles.texto, styles.smallCell]}>{index + 1}</Text>
                <Text style={[styles.cell, styles.texto]}>{estudiante.Nombre} {estudiante.Apellido_Paterno} {estudiante.Apellido_Materno}</Text>
                <View style={[styles.cell, styles.contenedorB]}>
                  <TouchableOpacity
                    style={[styles.editButton]}
                    onPress={() => mostrarCursos(estudiante.id)}
                  >
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <Text style={styles.modalTitle}>Cursos Inscritos</Text>
              <ScrollView>
